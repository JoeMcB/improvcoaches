...
class UsersController < ApplicationController
  ...
  # GET /users.json
  def index
    @users = current_city.coaches.joins(:schedule)
    
    # Apply filters
    case params[:filter]
    when 'recent'
      @users = @users.order('schedules.updated_at desc')
    when 'rated'
      @users = @users.where('rating > 0').order('rating desc, schedules.updated_at desc')
    else
      @users = @users.order('schedules.updated_at desc')
    end
